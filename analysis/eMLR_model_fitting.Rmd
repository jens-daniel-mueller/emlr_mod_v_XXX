...
for (i_basin in unique(GLODAP$basin)) {
  for (i_era in unique(GLODAP$era)) {
    ...
    for (i_gamma_slab in unique(GLODAP_basin_era$gamma_slab)) {
      ...
      lm_best_plot <- lm_best_plot %>%
        filter(basin == i_basin, gamma_slab == i_gamma_slab) 
      
      for (i_predictors in unique(lm_best_plot$predictors)) {
        i_predictors <- unique(lm_best_plot$predictors)[1]
        print(i_predictors)
        # extract one model definition
        i_lm <- lm_all %>%
          filter(predictors == i_predictors) %>%
