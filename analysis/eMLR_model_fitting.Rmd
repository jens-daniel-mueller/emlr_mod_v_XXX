...
```{r read_GLODAP-based_model_subsetting_used_for_fitting}
...
for (i_basin in unique(GLODAP$basin)) {
  for (i_era in unique(GLODAP$era)) {
    ...
    for (i_gamma_slab in unique(GLODAP_basin_era$gamma_slab)) {
      #i_gamma_slab <- unique(GLODAP_basin_era$gamma_slab)[15]
      print(i_gamma_slab)
      ...
      if (nrow(GLODAP_basin_era_slab) > 2) {
        for (i_predictors in unique(lm_all$predictors)) {
          ...
          i_cor_max <- GLODAP_basin_era_slab %>%
            dplyr::select(!!!syms(str_split(i_predictors, " ",
                                            simplify = TRUE))) %>%
...
lm_all_fitted_wide_eras <- lm_all_fitted_wide  %>%
  dplyr::select(basin, gamma_slab, model, era, rmse, aic, resid_max) %>%
  arrange(era) %>%
...
lm_best_target <- full_join(
  lm_best %>% dplyr::select(basin, gamma_slab, model, eras),
  eras_era)
...
`r paste(path_version_figures, "eMLR_diagnostics/", sep = "")`
