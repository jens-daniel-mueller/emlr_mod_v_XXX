...
# whether manually change the tref year
if (params_local$median_tref == "n") {
...
if ("phosphate" %in% params_local$MLR_predictors || "phosphate_star" %in% params_local$MLR_predictors) {
  GLODAP <- GLODAP %>%
    mutate(
      rCP_phosphate = -params_local$rCP * phosphate,
      talk_05 = -0.5 * talk,
      rNP_phosphate_05 = -0.5 * params_local$rNP * phosphate,
      cstar = b_cstar(tco2, phosphate, talk)
    )
  print("phosphate-based")
} else {
  GLODAP <- GLODAP %>%
    mutate(
      rCP_phosphate = -params_local$rCP * nitrate,
      talk_05 = -0.5 * talk,
      rNP_phosphate_05 = -0.5 * params_local$rNP * nitrate,
      cstar = b_cstar(tco2, nitrate, talk)
    )
  print("nitrate-based")
}
...
map +
  geom_raster(data = cant_3d %>% filter(depth == min(depth)),
...
if (params_local$median_tref == "n") {
  ...
  tref_temp <- unique(GLODAP$tref)
  
  GLODAP <- GLODAP %>% 
    mutate(tref = case_when(tref == tref_temp[2], tref$year[2],
                            tref == tref_temp[3], tref$year[3])) ####################
...
if (params_local$subsetting == "random") {
  # select relevant columns
  if (params_local$model_resolution == "annual") {
  GLODAP <- GLODAP %>%
    select(
      year,
    #  month,
  ...
  if (params_local$model_resolution == "month") {
  GLODAP <- GLODAP %>%
    select(
      year,
      month,
