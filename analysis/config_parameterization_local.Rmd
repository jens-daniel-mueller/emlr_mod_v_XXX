...
############### Some manual work before running the full reconstruction
# change "config_parameterization_local": 
#    !!!whenever no phosphate, Stoichiometricratio C:P and N:P based on predictor nitrate (solved)
#    model_resolution month or annual
#    nan_fill yes or no & flag_f add 9
#    basin separation
#    slab defined
#    depth and neutral density threshold
# change "eMLR_GLODAPv2_2020_subsetting": ()/_ann
# change "eMLR_model_fitting": remove multicolinearity combination (solved)
...
if ("phosphate" %in% MLR_predictors || "phosphate_star" %in% MLR_predictors) {
    # Stoichiometric ratio of C to P (Phosphate-based: 117)
    rCP = 117
    # Stoichiometric ratio of N to P (Phosphate-based: 16)
    rNP = 16
} else {
    # Stoichiometric ratio of C to N (Nitrate-based: 117/16)
    rCP = 117/16
    # Stoichiometric ratio of N to N (Nitrate-based: 1)
    rNP = 1
}
...
params_local <-
  lst(
    ...
    Version_ID = "v_XXX",
    # f flags accepted for GLODAP data (include 9 when use the nafilled subset to avoid data cleaning)
    flag_f = c(2, 0),  ## add 9
    ...
    # whether use median year as the tref  ##############need further modification based on Jens' method
    median_tref = "y",
    # ID for basins for MLR fits: 1, 2, 5, SO, AIP, SO_5
    MLR_basins = "1",
