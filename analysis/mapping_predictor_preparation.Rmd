...
predictors <- climatology %>%
  dplyr::select(lon, lat, depth, basin_AIP, gamma, 
...
predictors_surface <- climatology %>%
  dplyr::select(lon, lat, depth, basin_AIP, gamma, sal, temp, TCO2, TAlk, phosphate, silicate)
...
N_Atl <- N_Atl %>%
  dplyr::select(-c(basin, basin_AIP)) %>%
