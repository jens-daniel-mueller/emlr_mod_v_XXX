...
predictors <- climatology %>%
  select(lon, lat, depth, basin_AIP, gamma, sal, temp, phosphate, oxygen, aou = AOU, silicate)
...
predictors_surface <- climatology %>%
  select(lon, lat, depth, basin_AIP, gamma, sal, temp, TCO2 = tco2, TAlk = talk, phosphate, silicate)
...
N_Atl <- N_Atl %>% 
  ...
  pivot_longer(gamma:silicate, names_to = "parameter", values_to = "value")
